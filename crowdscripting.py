							...
							if ps:
								if ps.string == 'My blogs':
									...
									if re.findall('blogspot',blog):
										...
										#blog='https://'+g
										try:
											url=urllib2.urlopen(blog)
										except urllib2.URLError:
											print 'cannot open'
										else:
											content=url.read()
											soup= BeautifulSoup(content)
											print 'Posts for this blog'
											posts= soup.findAll("h3","post-title entry-title")
											for post in posts:
												try:
													c=post.find("a").string
													...
													if(pos_or_neg(c)):
														userposts.append(c)
				if(( userposts != []  or userblogs != []) and email!=""):
					db.scripters.update({"_id":user["_id"]},{'$set':{"username":username,"email":email,"blogs":userblogs,"posts":userposts}})
					print 'updated user '
				else:
